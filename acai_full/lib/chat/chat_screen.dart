...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _startRecord() async {
    ...
    if (!await _recorder.hasPermission()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Sem permissão para gravar')),
        );
      }
      return;
    }
    ...
    final path = p.join(dir.path, 'acai_audio_${DateTime.now().millisecondsSinceEpoch}.m4a');
    ...
    _recordTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted && _recordStarted != null) {
        setState(() {
          _recordSeconds = DateTime.now().difference(_recordStarted!).inSeconds;
        });
      }
    });
  }
  ...
  Future<void> _stopAndSendAudio() async {
    ...
    if (!await file.exists() || await file.length() < 500) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Áudio muito curto')),
        );
      }
      return;
    }
    ...
    await _sendFile(file, name, 'audio/mp4');
  }
              ...
              child: SafeArea(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      'Gravando ${_formatRecordTime(_recordSeconds)}',
                    ...
                    ElevatedButton.icon(
                      ...
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: const Color(0xFF6B21A8)),
